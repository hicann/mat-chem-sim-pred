...
本仓库面向材料、化学与流程工业的科学计算和预测优化场景，基于华为 CANN（Ascend C）建设可复用的软件资产，包括模型实现与适配、算子、数据处理与评测脚本、工具链、示例和文档。

仓库覆盖的典型任务包括分子模拟、材料性质预测、反应路径优化、工艺过程建模、过程控制和时序预测等。

核心定位：构建“**模拟 → 数据 → 预测 → 设计 → 优化**”的全链路材料与化学 AI 软件基础。
