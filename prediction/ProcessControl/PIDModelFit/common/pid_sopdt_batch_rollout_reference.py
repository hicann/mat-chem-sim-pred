...
def make_sopdt_batch_rollout_case(
    batch: int = 128,
    candidates: int = 1024,
    sim_steps: int = 2048,
    seed: int = 2031,
    ...
    tau1 = rng.uniform(8.0, 40.0, size=batch).astype(np.float32)
    tau2 = rng.uniform(2.0, 10.0, size=batch).astype(np.float32)
    gain = rng.uniform(0.6, 1.8, size=batch).astype(np.float32)
    dt = 1.0
    # Two stable real poles -> discrete SOPDT denominator z^2 - (p1+p2) z + p1 p2,
    # i.e. y[k+1] = a1*y[k] + a2*y[k-1] + b*u[k-delay] with a1=p1+p2, a2=-p1*p2.
    p1 = np.exp(-dt / tau1)
    p2 = np.exp(-dt / tau2)
    a1 = (p1 + p2).astype(np.float32)
    a2 = (-p1 * p2).astype(np.float32)
    b = (gain * (1.0 - p1) * (1.0 - p2)).astype(np.float32)
    ...
    return SopdtClosedLoopCase(a1, a2, b, delay, y0, sp, kp, ki, kd, dt, sim_steps)
...
def _simulate_candidate(
    ...
    for step in range(sim_steps):
        ...
        y_new = a1 * y + a2 * y_prev + b * delayed_u
        y_prev = y
        y = y_new
...
def benchmark_reference(batch: int, candidates: int, sim_steps: int, iters: int = 3) -> dict[str, float]:
    case = make_sopdt_batch_rollout_case(batch=batch, candidates=candidates, sim_steps=sim_steps)
    sopdt_batch_rollout_score(case)
