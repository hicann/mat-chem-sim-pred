...
def make_ipdt_batch_rollout_case(
    batch: int = 128,
    candidates: int = 1024,
    sim_steps: int = 2048,
    seed: int = 2029,
    ...
    dt = 1.0
    # Pure integrating plant y[k+1] = y[k] + b * u[k-delay]; b is the per-step
    # integration gain. Derived from a velocity-form first-order step so it stays
    # in a numerically reasonable range for the candidate sweep.
    b = (gain * (1.0 - np.exp(-dt / tau))).astype(np.float32)
    ...
    return IpdtClosedLoopCase(b, delay, y0, sp, kp, ki, kd, dt, sim_steps)
...
def ipdt_batch_rollout_score(
    case: IpdtClosedLoopCase,
...
def benchmark_reference(batch: int, candidates: int, sim_steps: int, iters: int = 3) -> dict[str, float]:
    case = make_ipdt_batch_rollout_case(batch=batch, candidates=candidates, sim_steps=sim_steps)
    ipdt_batch_rollout_score(case)
