...
int main(int argc, char** argv)
{
    ...
    const std::vector<float> actual = {1.0f, 2.0f, 3.0f, 4.0f};
    const std::vector<float> predicted = {1.0f, 1.0f, 3.0f, 5.0f};
    std::vector<float> metrics(static_cast<size_t>(batch * PID_RESIDUAL_DIAGNOSTICS_METRIC_COUNT), 0.0f);
    std::vector<float> autocorr(static_cast<size_t>(batch * max_lag), 0.0f);
    ...
    const uint64_t workspace_size = aclnnPidResidualDiagnosticsGetWorkspaceSize(batch, sample_count, max_lag);
    ...
    const int32_t ret = aclnnPidResidualDiagnostics(
        d_actual, d_predicted, d_metrics, d_autocorr, batch, sample_count, max_lag, workspace, workspace_size, stream);
    ...
    const bool passed = std::fabs(metrics[0] - 0.0f) < 1.0e-3f && std::fabs(metrics[2] - 0.5f) < 1.0e-3f &&
                        std::fabs(metrics[3] - 0.70710678f) < 1.0e-3f &&
    ...
    std::cout << "PidResidualDiagnostics smoke mean=" << metrics[0] << " mae=" << metrics[2]
              << " rmse=" << metrics[3] << " max_abs=" << metrics[4] << " dw=" << metrics[6]
              << " autocorr=[" << autocorr[0] << ", " << autocorr[1] << "]" << std::endl;
