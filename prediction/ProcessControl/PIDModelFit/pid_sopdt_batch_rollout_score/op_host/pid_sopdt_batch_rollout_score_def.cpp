...
namespace ops {
class PidSopdtBatchRolloutScore : public OpDef {
public:
    explicit PidSopdtBatchRolloutScore(const char* name) : OpDef(name)
    {
        this->Input("a1").ParamType(REQUIRED).DataType({DT_FLOAT}).Format({FORMAT_ND});
        this->Input("a2").ParamType(REQUIRED).DataType({DT_FLOAT}).Format({FORMAT_ND});
...
OP_ADD(PidSopdtBatchRolloutScore);
}  // namespace ops
