...
extern "C" uint32_t aclrtlaunch_pid_sopdt_batch_rollout_score_local_kernel(
    uint32_t blockDim, aclrtStream stream, void* a1, void* a2, void* b, void* delay, void* y0, void* sp, void* kp,
    void* ki, void* kd, void* partial_result, void* partial_idx, void* tiling);

extern "C" uint32_t aclrtlaunch_pid_sopdt_batch_rollout_score_final_kernel(
...
extern "C" int32_t aclnnPidSopdtBatchRolloutScore(
    void* a1, void* a2, void* b, void* delay, void* y0, void* sp, void* kp, void* ki, void* kd, void* best_result,
...
{
    if (a1 == nullptr || a2 == nullptr || b == nullptr || delay == nullptr || y0 == nullptr || sp == nullptr || kp == nullptr ||
        ki == nullptr || kd == nullptr || best_result == nullptr || best_idx == nullptr || workspace == nullptr ||
        stream == nullptr) {
        return ACL_ERROR_INVALID_PARAM;
    }
    if (batch <= 0 || candidates <= 0 || sim_steps <= 0 ||
        workspace_size < aclnnPidSopdtBatchRolloutScoreGetWorkspaceSize(batch, candidates, sim_steps, candidate_tile)) {
        return ACL_ERROR_INVALID_PARAM;
    }
    ...
    for (uint64_t tile = 0U; tile < tile_count; ++tile) {
        ...
        const uint32_t launch_ret = aclrtlaunch_pid_sopdt_batch_rollout_score_local_kernel(
            core_num, reinterpret_cast<aclrtStream>(stream), a1, a2, b, delay, y0, sp, kp, ki, kd, metrics,
    ...
    const uint32_t launch_ret = aclrtlaunch_pid_sopdt_batch_rollout_score_final_kernel(
        core_num, reinterpret_cast<aclrtStream>(stream), kp, ki, kd, metrics, best_result, best_idx,
        final_tiling_device);
    return static_cast<int32_t>(launch_ret);
}

extern "C" uint64_t aclnnPidSopdtBatchRolloutScoreGetWorkspaceSize(
