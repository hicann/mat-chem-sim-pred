...
ClosedLoopCase MakeCase(int64_t batch, int64_t candidates, int64_t sim_steps)
{
    ...
    data.a1.assign(static_cast<size_t>(batch), 0.0f);
    data.a2.assign(static_cast<size_t>(batch), 0.0f);
    ...
    for (int64_t i = 0; i < batch; ++i) {
        const float tau1 = 8.0f + 0.2f * static_cast<float>(i % 160);
        const float tau2 = 3.0f + 0.1f * static_cast<float>(i % 90);
        const float gain = 0.6f + 0.01f * static_cast<float>(i % 120);
        // Two stable real poles -> SOPDT denominator z^2 - (p1+p2) z + p1 p2.
        const float p1 = std::exp(-data.sample_interval / tau1);
        const float p2 = std::exp(-data.sample_interval / tau2);
        data.a1[static_cast<size_t>(i)] = p1 + p2;
        data.a2[static_cast<size_t>(i)] = -p1 * p2;
        data.b[static_cast<size_t>(i)] = gain * (1.0f - p1) * (1.0f - p2);
...
{
    const size_t out = static_cast<size_t>(loop * PID_SOPDT_BATCH_ROLLOUT_RESULT_COUNT);
    ...
    std::vector<int32_t>& best_idx)
{
    for (int64_t loop = begin; loop < end; ++loop) {
        const float a1 = data.a1[static_cast<size_t>(loop)];
        const float a2 = data.a2[static_cast<size_t>(loop)];
        ...
        for (int64_t candidate = 0; candidate < data.candidates; ++candidate) {
            ...
            for (int64_t step = 0; step < data.sim_steps; ++step) {
                ...
                const float y_new = a1 * y + a2 * y_prev + b * delayed_control;  // SOPDT 2nd order
                y_prev = y;
                y = y_new;
    ...
    const ClosedLoopCase& data, int threads, std::vector<float>& best_result, std::vector<int32_t>& best_idx)
{
    ...
    best_result.assign(static_cast<size_t>(data.batch * PID_SOPDT_BATCH_ROLLOUT_RESULT_COUNT), 0.0f);
...
{
    const int worker_count = static_cast<int>(std::min<int64_t>(std::max(1, threads), data.batch));
    std::vector<float> best_result(static_cast<size_t>(data.batch * PID_SOPDT_BATCH_ROLLOUT_RESULT_COUNT), 0.0f);
...
{
    NpuResult result;
    result.best_result.assign(static_cast<size_t>(data.batch * PID_SOPDT_BATCH_ROLLOUT_RESULT_COUNT), 0.0f);
    ...
    d_a1 = MallocDevice(data.a1.size() * sizeof(float));
    d_a2 = MallocDevice(data.a2.size() * sizeof(float));
    ...
    const uint64_t workspace_size =
        aclnnPidSopdtBatchRolloutScoreGetWorkspaceSize(data.batch, data.candidates, data.sim_steps, data.candidate_tile);
    workspace = MallocDevice(workspace_size);

    auto copy_inputs = [&]() {
        CHECK_ACL(aclrtMemcpy(d_a1, data.a1.size() * sizeof(float), data.a1.data(), data.a1.size() * sizeof(float),
                              ACL_MEMCPY_HOST_TO_DEVICE));
        CHECK_ACL(aclrtMemcpy(d_a2, data.a2.size() * sizeof(float), data.a2.data(), data.a2.size() * sizeof(float),
                              ACL_MEMCPY_HOST_TO_DEVICE));
    ...
    auto launch_once = [&]() {
        const int32_t ret = aclnnPidSopdtBatchRolloutScore(
            d_a1, d_a2, d_b, d_delay, d_y0, d_sp, d_kp, d_ki, d_kd, d_best_result, d_best_idx, data.batch, data.candidates,
...
ErrorStats ComputeQualityErrorStats(const std::vector<float>& lhs, const std::vector<float>& rhs)
{
    ...
    const size_t loop_count = lhs.size() / PID_SOPDT_BATCH_ROLLOUT_RESULT_COUNT;
    for (size_t loop = 0; loop < loop_count; ++loop) {
        const size_t base = loop * PID_SOPDT_BATCH_ROLLOUT_RESULT_COUNT;
...
{
    int count = 0;
    const size_t loop_count = values.size() / PID_SOPDT_BATCH_ROLLOUT_RESULT_COUNT;
    for (size_t loop = 0; loop < loop_count; ++loop) {
        if (values[loop * PID_SOPDT_BATCH_ROLLOUT_RESULT_COUNT] == 0.0f) {
            ++count;
        }
    }
    return count;
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cout << "op=PidSopdtBatchRolloutScore batch=" << batch << " candidates=" << candidates
