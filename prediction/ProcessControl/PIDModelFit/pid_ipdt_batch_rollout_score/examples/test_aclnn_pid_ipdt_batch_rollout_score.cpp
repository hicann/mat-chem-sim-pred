...
int main(int argc, char** argv)
{
    ...
    std::vector<float> best_result(static_cast<size_t>(batch * PID_IPDT_BATCH_ROLLOUT_RESULT_COUNT), 0.0f);
    ...
    const uint64_t workspace_size =
        aclnnPidIpdtBatchRolloutScoreGetWorkspaceSize(batch, candidates, sim_steps, candidate_tile);
    ...
    const int32_t ret = aclnnPidIpdtBatchRolloutScore(
        d_b, d_delay, d_y0, d_sp, d_kp, d_ki, d_kd, d_best_result, d_best_idx, batch, candidates, sim_steps,
    ...
    std::cout << "PidIpdtBatchRolloutScore smoke best_idx=" << best_idx[0] << " best_score=" << best_result[0]
