...
extern "C" __global__ __aicore__ void pid_ipdt_batch_rollout_score_local_kernel(
    GM_ADDR b, GM_ADDR delay, GM_ADDR y0, GM_ADDR sp, GM_ADDR kp, GM_ADDR ki, GM_ADDR kd,
    ...
    for (uint32_t loop = begin; loop < end; ++loop) {
        ...
        for (uint32_t sub = 0U; sub < candidate_count; sub += kLane) {
            ...
            for (uint32_t step = 0U; step < sim_steps; ++step) {
                ...
                // y = y + model_b * delayed_control  (IPDT integrator, a = 1)
                Add(y, y, dtmp, cnt);
...
extern "C" __global__ __aicore__ void pid_ipdt_batch_rollout_score_final_kernel(
