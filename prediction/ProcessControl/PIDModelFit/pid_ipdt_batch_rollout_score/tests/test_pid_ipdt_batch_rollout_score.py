...
from pid_ipdt_batch_rollout_reference import (
    RESULT_NAMES,
    ipdt_batch_rollout_score,
    make_ipdt_batch_rollout_case,
)


def test_ipdt_batch_rollout_shape_and_sanity():
    case = make_ipdt_batch_rollout_case(batch=4, candidates=8, sim_steps=64)
    results, best_idx = ipdt_batch_rollout_score(case)
...
def test_ipdt_batch_rollout_prefers_some_control_over_zero_gain():
    case = make_ipdt_batch_rollout_case(batch=1, candidates=3, sim_steps=96)
