...
#include "acl/acl.h"
#include "pid_fopdt_basis_gemm_fit_host.h"
...
NpuTiming RunNpu(const ReduceCase& data, int device_id, int iters, int warmup)
{
    ...
    const uint64_t workspace_size = aclnnPidFopdtBasisGemmFitGetWorkspaceSize(data.batch, data.candidates);
    ...
    auto launch_once = [&]() {
        const int32_t ret = aclnnPidFopdtBasisGemmFit(
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cout << "op=PidFopdtBasisGemmFitReduce batch=" << batch << " candidates=" << candidates
