...
int main(int argc, char** argv)
{
    ...
    const std::vector<float> a = {0.92f};
    const std::vector<float> b = {0.08f};
    ...
    std::vector<float> best_result(static_cast<size_t>(batch * PID_FOPDT_BATCH_ROLLOUT_RESULT_COUNT), 0.0f);
    ...
    const uint64_t workspace_size =
        aclnnPidFopdtBatchRolloutScoreGetWorkspaceSize(batch, candidates, sim_steps, candidate_tile);
    ...
    const int32_t ret = aclnnPidFopdtBatchRolloutScore(
        d_a, d_b, d_delay, d_y0, d_sp, d_kp, d_ki, d_kd, d_best_result, d_best_idx, batch, candidates, sim_steps,
    ...
    std::cout << "PidFopdtBatchRolloutScore smoke best_idx=" << best_idx[0] << " best_score=" << best_result[0]
