...
ClosedLoopCase MakeCase(int64_t batch, int64_t candidates, int64_t sim_steps)
{
    ...
    data.a.assign(static_cast<size_t>(batch), 0.0f);
...
{
    const size_t out = static_cast<size_t>(loop * PID_FOPDT_BATCH_ROLLOUT_RESULT_COUNT);
    ...
    std::vector<int32_t>& best_idx)
{
    for (int64_t loop = begin; loop < end; ++loop) {
        const float a = data.a[static_cast<size_t>(loop)];
    ...
    const ClosedLoopCase& data, int threads, std::vector<float>& best_result, std::vector<int32_t>& best_idx)
{
    ...
    best_result.assign(static_cast<size_t>(data.batch * PID_FOPDT_BATCH_ROLLOUT_RESULT_COUNT), 0.0f);
...
{
    const int worker_count = static_cast<int>(std::min<int64_t>(std::max(1, threads), data.batch));
    std::vector<float> best_result(static_cast<size_t>(data.batch * PID_FOPDT_BATCH_ROLLOUT_RESULT_COUNT), 0.0f);
...
{
    NpuResult result;
    result.best_result.assign(static_cast<size_t>(data.batch * PID_FOPDT_BATCH_ROLLOUT_RESULT_COUNT), 0.0f);
    ...
    d_a = MallocDevice(data.a.size() * sizeof(float));
    ...
    const uint64_t workspace_size =
        aclnnPidFopdtBatchRolloutScoreGetWorkspaceSize(data.batch, data.candidates, data.sim_steps, data.candidate_tile);
    workspace = MallocDevice(workspace_size);

    auto copy_inputs = [&]() {
        CHECK_ACL(aclrtMemcpy(d_a, data.a.size() * sizeof(float), data.a.data(), data.a.size() * sizeof(float),
    ...
    auto launch_once = [&]() {
        const int32_t ret = aclnnPidFopdtBatchRolloutScore(
            d_a, d_b, d_delay, d_y0, d_sp, d_kp, d_ki, d_kd, d_best_result, d_best_idx, data.batch, data.candidates,
...
ErrorStats ComputeQualityErrorStats(const std::vector<float>& lhs, const std::vector<float>& rhs)
{
    ...
    const size_t loop_count = lhs.size() / PID_FOPDT_BATCH_ROLLOUT_RESULT_COUNT;
    for (size_t loop = 0; loop < loop_count; ++loop) {
        const size_t base = loop * PID_FOPDT_BATCH_ROLLOUT_RESULT_COUNT;
...
{
    int count = 0;
    const size_t loop_count = values.size() / PID_FOPDT_BATCH_ROLLOUT_RESULT_COUNT;
    for (size_t loop = 0; loop < loop_count; ++loop) {
        if (values[loop * PID_FOPDT_BATCH_ROLLOUT_RESULT_COUNT] == 0.0f) {
            ++count;
        }
    }
    return count;
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cout << "op=PidFopdtBatchRolloutScore batch=" << batch << " candidates=" << candidates
