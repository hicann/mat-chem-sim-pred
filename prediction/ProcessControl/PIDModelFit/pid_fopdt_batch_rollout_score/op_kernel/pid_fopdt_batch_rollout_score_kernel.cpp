...
extern "C" __global__ __aicore__ void pid_fopdt_batch_rollout_score_local_kernel(
    GM_ADDR a, GM_ADDR b, GM_ADDR delay, GM_ADDR y0, GM_ADDR sp, GM_ADDR kp, GM_ADDR ki, GM_ADDR kd,
    GM_ADDR partial_result, GM_ADDR partial_idx, GM_ADDR tiling)
{
    ...
    a_gm.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(a));
    ...
    // block 11 is intentionally unused: the response error is carried in `error`.
    ...
    for (uint32_t loop = begin; loop < end; ++loop) {
        const float model_a = a_gm.GetValue(loop);
        ...
        for (uint32_t sub = 0U; sub < candidate_count; sub += kLane) {
            ...
            for (uint32_t step = 0U; step < sim_steps; ++step) {
                ...
                // y = model_a * y + model_b * delayed_control
                Muls(ytmp, y, model_a, cnt);
                Add(y, ytmp, dtmp, cnt);
...
extern "C" __global__ __aicore__ void pid_fopdt_batch_rollout_score_final_kernel(
