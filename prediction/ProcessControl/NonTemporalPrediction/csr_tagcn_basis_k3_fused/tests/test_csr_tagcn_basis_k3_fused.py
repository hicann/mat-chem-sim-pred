...
import numpy as np
from reference import csr_tagcn_basis_k3
...
def test_two_node_three_hop_basis() -> None:
    ...
    actual = csr_tagcn_basis_k3(row_ptr, source, norm, features)
    ...
    np.testing.assert_allclose(actual[3], features[::-1])
...
def test_negative_norm_is_supported() -> None:
    result = csr_tagcn_basis_k3(
        np.array([0, 1], dtype=np.int32),
        np.array([0], dtype=np.int32),
        np.array([-0.5], dtype=np.float32),
        np.array([[2.0]], dtype=np.float32),
    )
    np.testing.assert_allclose(result[:, 0, 0], [2.0, -1.0, 0.5, -0.25])


def test_invalid_csr_is_rejected() -> None:
    check_basis_invalid(csr_tagcn_basis_k3)


def test_non_finite_norm_is_rejected() -> None:
    check_basis_nonfinite(csr_tagcn_basis_k3)
