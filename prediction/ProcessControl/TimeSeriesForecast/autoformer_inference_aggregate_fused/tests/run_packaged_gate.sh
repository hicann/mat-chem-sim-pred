...
cp "$OP_ROOT/msopgen/autoformer_inference_aggregate_fused_msopgen.json" "$WORK_ROOT/schema.json"
...
mv "$PROJECT/op_host/autoformer_inference_aggregate_fused.cpp" \
    "$PROJECT/op_host/autoformer_inference_aggregate_fused.generated.cpp.stub"
cp "$OP_ROOT/op_host/autoformer_inference_aggregate_fused_host.cpp" "$PROJECT/op_host/"
cp "$OP_ROOT/op_host/autoformer_inference_aggregate_fused_def.cpp" "$PROJECT/op_host/"
cp "$OP_ROOT/op_host/autoformer_inference_aggregate_fused_tiling.h" "$PROJECT/op_host/"
cp "$OP_ROOT/op_kernel/autoformer_inference_aggregate_fused_kernel.cpp" \
    "$PROJECT/op_kernel/autoformer_inference_aggregate_fused.cpp"
...
g++ -std=c++17 -O2 "$OP_ROOT/examples/test_aclnn_autoformer_inference_aggregate_fused.cpp" \
    ...
    -o "$WORK_ROOT/test_aclnn_autoformer_inference_aggregate_fused"
...
LD_LIBRARY_PATH="$PROJECT/build_formal/autogen:$PROJECT/build_out/op_api/lib:${LD_LIBRARY_PATH:-}" \
    "$WORK_ROOT/test_aclnn_autoformer_inference_aggregate_fused"
