#!/usr/bin/env python3
"""coRNN encoder E2E: replace the per-timestep recurrence loop with CornnScanFused.
...
DEV = "npu:0"
ROOT = "/home/ql2025/work/tslib_cann_ops_dev/build/msopgen_cornn_scan_fused/build_out"
...
def init_op():
    ...
    op.aclnnCornnScanFusedGetWorkspaceSize.argtypes = [c_void_p]*4 + [POINTER(c_uint64), POINTER(c_void_p)]
    op.aclnnCornnScanFusedGetWorkspaceSize.restype = c_int
    op.aclnnCornnScanFused.argtypes = [c_void_p, c_uint64, c_void_p, c_void_p]
    op.aclnnCornnScanFused.restype = c_int
    return acl, op
...
def custom_cornn(x, Wcat, bcat):
    ...
    r = _OP.aclnnCornnScanFusedGetWorkspaceSize(desc[0], desc[1], desc[2], desc[3], byref(ws), byref(ex))
    ...
    r = _OP.aclnnCornnScanFused(c_void_p(0 if wbuf is None else wbuf.data_ptr()), ws.value, ex,
                              c_void_p(torch.npu.current_stream().npu_stream))
    if r: raise RuntimeError(f"aclnnCornnScanFused ret={r}")
    torch.npu.synchronize()
    for d in desc: _ACL.aclDestroyTensor(d)
    return out


DT, GAMMA, EPS = 0.042, 1.0, 1.0


def torch_cornn_loop(x, Wcat, bcat, H):
    """Faithful coRNN IMEX per-timestep loop (framework baseline). x[B,L,IN] -> out[B,L,H].
    Wcat[2H+IN,H]: rows[0:H]=Wy, rows[H:2H]=Wz, rows[2H:2H+IN]=V. bcat[H]."""
    B, L, IN = x.shape
    y = torch.zeros(B, H, device=DEV); z = torch.zeros(B, H, device=DEV)
    outs = []
    for s in range(L):
        xs = x[:, s, :]
        zc = torch.cat([y, z, xs], dim=-1)
        pre = zc @ Wcat + bcat
        z = z + DT * (torch.tanh(pre) - GAMMA * y - EPS * z)
        y = y + DT * z
        outs.append(y)
...
def torch_encoder(u0, Ws, Bs, H):
    ...
    for layer in range(len(Ws)):
        a = torch_cornn_loop(a, Ws[layer], Bs[layer], H)
    return a
...
def fused_encoder(u0, Ws, Bs, H):
    a = u0
    for layer in range(len(Ws)):
        a = custom_cornn(a, Ws[layer], Bs[layer])
    return a
...
def main():
    ...
    with torch.no_grad():
        a_ref = u0
        for layer in range(layers):
            t_out = torch_cornn_loop(a_ref, Ws[layer], Bs[layer], H)
            o_out = custom_cornn(a_ref, Ws[layer], Bs[layer])
        ...
        ms_torch_1 = bench(lambda: torch_cornn_loop(u0, Ws[0], Bs[0], H), warmup=2, repeat=10)
        ms_fused_1 = bench(lambda: custom_cornn(u0, Ws[0], Bs[0]), warmup=3, repeat=20)
    ...
    with open("/home/ql2025/work/tslib_cann_ops_dev/docs/cornn_scan_fused_e2e_results.json", "w") as f:
