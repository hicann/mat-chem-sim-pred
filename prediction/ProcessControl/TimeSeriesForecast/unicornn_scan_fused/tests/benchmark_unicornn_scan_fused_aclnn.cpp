...
#include "acl/acl.h"
#include "aclnn_unicornn_scan_fused.h"
...
// Faithful CPU oracle of one UnICORNN cell (Rusch & Mishra ICML'21), per timestep:
//   arg   = w (.) y + V x_s + b   (w diagonal recurrent)
//   sig_c = sigmoid(c)
//   z     = z - dt*sig_c*( tanh(arg) + alpha*y )
//   y     = y + dt*sig_c*z
// x   : [B, L, IN]
// wt  : [IN+2, H]  column-packed transposed weight:
//         wt[0:IN,:]=V, wt[IN,:]=w (diag), wt[IN+1,:]=c (control)
// bias: [H] = b
// out : [B, L, H]  (= y)
void Ref(int64_t B, int64_t L, int64_t IN, int64_t H,
  ...
  std::vector<float> y(H), z(H), sigc(H), vx(H);
  for (int64_t m = 0; m < H; ++m) sigc[m] = Sigmoid(wt[(IN + 1) * H + m]);
  for (int64_t b = 0; b < B; ++b) {
    ...
    for (int64_t s = 0; s < L; ++s) {
      for (int64_t m = 0; m < H; ++m) {
        float acc = 0.0f;
        for (int64_t k = 0; k < IN; ++k) acc += x[((b * L) + s) * IN + k] * wt[k * H + m];
        vx[m] = acc;
      }
      for (int64_t m = 0; m < H; ++m) {
        const float arg = wt[IN * H + m] * y[m] + vx[m] + bias[m];
        z[m] = z[m] - DT * sigc[m] * (std::tanh(arg) + ALPHA * y[m]);
        y[m] = y[m] + DT * sigc[m] * z[m];
        out[((b * L) + s) * H + m] = y[m];
      }
    }
  }
...
int RunCase(int64_t B, int64_t L, int64_t IN, int64_t H, int repeats, bool check) {
  ...
  CHECK_ACLNN(aclnnUnicornnScanFusedGetWorkspaceSize(x_t, w_t, b_t, out_t, &workspace_size, &executor));
  ...
  CHECK_ACLNN(aclnnUnicornnScanFused(workspace, workspace_size, executor, stream));
  ...
  if (repeats > 0) {
    ...
    for (int i = 0; i < repeats; ++i) {
      ...
      CHECK_ACLNN(aclnnUnicornnScanFusedGetWorkspaceSize(x_t, w_t, b_t, out_t, &ws, &ex));
      ...
      CHECK_ACLNN(aclnnUnicornnScanFused(workspace, ws, ex, stream));
    }
    CHECK_ACL(aclrtSynchronizeStream(stream));
