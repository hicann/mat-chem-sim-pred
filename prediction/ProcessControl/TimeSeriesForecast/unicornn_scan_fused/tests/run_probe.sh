...
g++ -O2 -std=c++17 -I"$CANN/include" -I"$PWD/$M/op_api/include" \
  cann_ops/unicornn_scan_fused/tests/unicornn_scan_fused_probe.cpp \
  -o build/unicornn_scan_fused_probe \
  -L"$PWD/$M/op_api/lib" -lcust_opapi \
  -L"$CANN/lib64" -lascendcl -lnnopbase -ldl
echo ===RUN PROBE===
# args: B L IN H repeats check
./build/unicornn_scan_fused_probe 4 7 16 16 20 1 2>&1 | tail -25
./build/unicornn_scan_fused_probe 8 33 21 32 20 1 2>&1 | tail -25
./build/unicornn_scan_fused_probe 16 64 11 64 20 1 2>&1 | tail -25
./build/unicornn_scan_fused_probe 8 50 64 64 20 1 2>&1 | tail -25
