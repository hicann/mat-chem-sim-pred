#!/usr/bin/env python3
"""UnICORNN encoder E2E: replace the per-timestep recurrence loop with UnicornnScanFused.
...
Weight convention (matches kernel + probe): Wcat is [IN+2, H]:
rows[0:IN]=V (input projection), row[IN]=w (diagonal recurrent), row[IN+1]=c (control).
bcat=[b]. Hidden-hidden coupling is diagonal; only V x_s is a matmul.
"""
import ctypes
import glob
import importlib
...
DEV = "npu:0"
ROOT = "/home/ql2025/work/tslib_cann_ops_dev/build/msopgen_unicornn_scan_fused/build_out"
...
def init_op():
    ...
    op.aclnnUnicornnScanFusedGetWorkspaceSize.argtypes = [c_void_p]*4 + [POINTER(c_uint64), POINTER(c_void_p)]
    op.aclnnUnicornnScanFusedGetWorkspaceSize.restype = c_int
    op.aclnnUnicornnScanFused.argtypes = [c_void_p, c_uint64, c_void_p, c_void_p]
    op.aclnnUnicornnScanFused.restype = c_int
    return acl, op
...
def custom_unicornn(x, Wcat, bcat):
    """x[B,L,IN] Wcat[IN+2,H] bcat[H] -> out[B,L,H]."""
    ...
    r = _OP.aclnnUnicornnScanFusedGetWorkspaceSize(desc[0], desc[1], desc[2], desc[3], byref(ws), byref(ex))
    ...
    r = _OP.aclnnUnicornnScanFused(c_void_p(0 if wbuf is None else wbuf.data_ptr()), ws.value, ex,
                              c_void_p(torch.npu.current_stream().npu_stream))
    if r: raise RuntimeError(f"aclnnUnicornnScanFused ret={r}")
...
def torch_unicornn_loop(x, Wcat, bcat, H):
    """Faithful UnICORNN per-timestep loop (framework baseline). x[B,L,IN] -> out[B,L,H].
    Wcat[IN+2,H]: rows[0:IN]=V, row[IN]=w (diag recurrent), row[IN+1]=c (control). bcat[H]=b."""
    B, L, IN = x.shape
    V = Wcat[0:IN, :]; w = Wcat[IN, :]; c = Wcat[IN + 1, :]
    sigc = torch.sigmoid(c)
    y = torch.zeros(B, H, device=DEV); z = torch.zeros(B, H, device=DEV)
    outs = []
    for s in range(L):
        xs = x[:, s, :]
        arg = w * y + xs @ V + bcat
        z = z - DT * sigc * (torch.tanh(arg) + ALPHA * y)
        y = y + DT * sigc * z
        outs.append(y)
...
def torch_encoder(u0, Ws, Bs, H):
    ...
    for layer in range(len(Ws)):
        a = torch_unicornn_loop(a, Ws[layer], Bs[layer], H)
    return a
...
def fused_encoder(u0, Ws, Bs, H):
    a = u0
    for layer in range(len(Ws)):
        a = custom_unicornn(a, Ws[layer], Bs[layer])
    return a
...
def main():
    ...
    for layer in range(layers):
        ...
        Bs.append(((torch.rand(H, device=DEV) * 2 - 1) * stdv).contiguous())

    with torch.no_grad():
        a_ref = u0
        for layer in range(layers):
            t_out = torch_unicornn_loop(a_ref, Ws[layer], Bs[layer], H)
            o_out = custom_unicornn(a_ref, Ws[layer], Bs[layer])
        ...
        ms_torch_1 = bench(lambda: torch_unicornn_loop(u0, Ws[0], Bs[0], H), warmup=2, repeat=10)
        ms_fused_1 = bench(lambda: custom_unicornn(u0, Ws[0], Bs[0]), warmup=3, repeat=20)
    ...
    with open("/home/ql2025/work/tslib_cann_ops_dev/docs/unicornn_scan_fused_e2e_results.json", "w") as f:
